...
phenotype_list
setwd("D:/A-MR/exposure/orginData")
getwd()
    ...
    tryCatch(
        {
            ...
            for (method in methods) {
                new_columns <- c(new_columns, paste0(c("pval", "b", "se"), "_", method))
            }
            # rm(new_columns)
            # 将方法名称转换为列名
            temp_data <- mrdat %>%
                pivot_wider(names_from = method, values_from = c("pval", "b", "se")) %>%
            ...
            replace_na_with_unique <- function(x) {
                ...
                if (length(unique_values) == 0) { # 如果没有非NA值
                    return(NA)
                } else {
            ...
            temp_data <- temp_data %>%
                mutate(Exposure = Exposure, ID.exposure = ID.exposure, nsnp = nsnp) %>%
...
View(mr_output)
write.csv(mr_output, "mr_output.csv", row.names = F)
    ...
    tryCatch(
        {
            ...
            hetero <- read.csv(paste0("hetero_", phenotype, ".csv"), header = T)

            ivw <- filter(hetero, method == "Inverse variance weighted")$Q_pval
            egger <- filter(hetero, method == "MR Egger")$Q_pval

            temp_data <- pleio %>%
                select(
                    Exposure = exposure, ID.exposure = id.exposure,
                    egger_intercept, intercept_se = se, intercept_pval = pval
                ) %>%
                mutate(
                    Q_pval_ivw = ivw,
                    Q_pval_egger = egger
...
View(ph_output)
write.csv(ph_output, "ph_output.csv", row.names = F)
...
for (phenotype in phenotype_list) {
    tryCatch(
        {
            snp <- read.csv(paste0(phenotype, ".csv"), header = T) %>% select(SNP)
            temp_data <- confuse[confuse$SNP %in% snp$SNP, ] %>% mutate(Exposure = phenotype, .before = 1)
